...
function handlePrimitive(primitive, { accessors, bufferViews }, ctx) {
  let geometryProps = {};
...
function handleMesh({ primitives, weights }, gltf, ctx, renderer) {
...
function handleNode(
  ...
  if (includeCameras && Number.isInteger(node.camera)) {
    const camera = gltf.cameras[node.camera];
    const enabled = enabledCameras.includes(node.camera);
  ...
  for (let buffer of json.buffers) {
    if (isBinary) {
      ...
      if (isBase64(buffer.uri)) {
        ...
        buffer._data = await loadArrayBuffer([basePath, buffer.uri].join("/"));
      }
    }
  }
