 ...
 * regular vertex attributes, each marked `stepMode: "instance"`. The three
 * spec semantics would step per-instance anyway once
 * loaders/glTF/pex-renderer.ts renames them to offsets/rotations/scales (see
 * the name match in systems/geometry.ts); a custom attribute has no such name,
 * so the descriptor has to say so itself.
