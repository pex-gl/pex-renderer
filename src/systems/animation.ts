...
function updateAnimation(
  ...
  if (animation.playing) {
    const animationLength = animation.duration || channels[0]!.input.at(-1)!;
    const now = performance.now();
    deltaTime ||= (now - animation.prevTime) / 1000;

    animation.prevTime = now;
    animation.time = (animation.time ?? 0) + deltaTime;
    ...
    if (prevIndex !== undefined) {
      switch (channel.interpolation) {
        case "STEP":
          ...
          break;
        case "CUBICSPLINE": {
          // Same component-wise operations either way; the two modules differ
          // only in how many components they touch.
          const vec = (isRotation ? vec4 : vec3) as typeof vec3;
          const tt = t * t;
          const ttt = tt * t;
      ...
      const transform = channel.target.transform!;

      if (isRotation) {
        quat.set(transform.rotation!, TEMP_QUAT);
        transform.dirty = true;
      } else
        switch (channel.path) {
          case "translation": {
            vec3.set(transform.position!, TEMP_VEC3);
            transform.dirty = true;

            break;
          }
          case "scale": {
            vec3.set(transform.scale!, TEMP_VEC3);
            transform.dirty = true;
...
export default () => ({
  ...
  updateAnimation,
  update(entities: Entity[], { deltaTime }: SystemUpdateOptions = {}) {
