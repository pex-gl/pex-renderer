...
import CONVOLVE_OCT_MAP_ATLAS_TO_OCT_MAP_FRAG from './glsl/ConvolveOctMapAtlasToOctMap.frag'
  ...
  constructor(opts) {
    ...
    const cubemapToOctMap = {
      ...
      pipeline: ctx.pipeline({
        vert: FULLSCREEN_QUAD_VERT,
        frag: CUBEMAP_TO_OCTMAP_FRAG
      }),
      attributes: {
    ...
    const convolveOctmapAtlasToOctMap = {
      ...
      pipeline: ctx.pipeline({
        vert: FULLSCREEN_QUAD_VERT,
        frag: CONVOLVE_OCT_MAP_ATLAS_TO_OCT_MAP_FRAG
      }),
      attributes: {
        aPosition: ctx.vertexBuffer(quadPositions),
        aTexCoord: ctx.vertexBuffer(quadTexCoords)
    ...
    const blitToOctMapAtlasCmd = {
      ...
      pipeline: ctx.pipeline({
        vert: FULLSCREEN_QUAD_VERT,
        frag: BLIT_TO_OCT_MAP_ATLAS_FRAG
      }),
      uniforms: {
        uSource: octMap,
    ...
    const downsampleFromOctMapAtlasCmd = {
      ...
      pipeline: ctx.pipeline({
        vert: FULLSCREEN_QUAD_VERT,
        frag: DOWNSAMPLE_FROM_OCT_MAP_ATLAS_FRAG
      }),
      uniforms: {
        uSource: octMapAtlas,
        uSourceSize: octMapAtlas.width
    ...
    const prefilterFromOctMapAtlasCmd = {
      ...
      pipeline: ctx.pipeline({
        vert: FULLSCREEN_QUAD_VERT,
        frag: PREFILTER_FROM_OCT_MAP_ATLAS_FRAG
      }),
      uniforms: {
        uSource: octMapAtlas,
        uSourceSize: octMapAtlas.width,
    ...
    function blitToOctMapAtlasLevel(mipmapLevel, roughnessLevel, sourceRegionSize) {
      ...
      const hOffset =
        2 * roughnessLevelWidth - 2 ** (Math.log2(2 * roughnessLevelWidth) - mipmapLevel)
...
export default opts => new ReflectionProbe(opts)
