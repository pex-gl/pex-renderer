...
vec3 getPositionVSOld(vec2 uv) {
  ...
  float depth = readDepth(sGBuffer, uv);
...
// #endif
  ...
  // gl_FragColor.rgb = vec3(radiusWS / 10.0);
  // gl_FragColor = vec4(originVS, 1.0);
  // gl_FragColor = vec4(vec3(randomPatternRotationAngle), 1.0);
}
