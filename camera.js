...
import POSTPROCESS_VERT from './glsl/Postprocess.vert'
import POSTPROCESS_FRAG from './glsl/Postprocess.frag'
import SAO_FRAG from './glsl/SAO.frag'
import BILATERAL_BLUR_FRAG from './glsl/BilateralBlur.frag'
import THRESHOLD_FRAG from './glsl/Threshold.frag'
import BLOOM_FRAG from './glsl/Bloom.frag'

const ssaoKernel = []
for (let i = 0; i < 64; i++) {
  const sample = [random.float() * 2 - 1, random.float() * 2 - 1, random.float(), 1]
...
for (let j = 0; j < 128 * 128; j++) {
  let noiseSample = [random.float() * 2 - 1, random.float() * 2 - 1, 0, 1]
